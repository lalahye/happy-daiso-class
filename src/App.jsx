  ...
  const spendPoints=async (n,reason='포인트 사용')=>{
    ...
    try{
      await runTransaction(db,async tx=>{
        ...
        if(!userSnap.exists())throw new Error('학생 정보를 찾을 수 없어요.');
        const current=Number(userSnap.data()?.points)||0;
        if(current<amount)throw new Error('포인트가 부족해요.');
        nextBalance=current-amount;
        tx.update(userRef,{points:nextBalance});
        tx.set(txRef,{uid,amount:-amount,reason,balanceAfter:nextBalance,createdAt:serverTimestamp()});
      });
      setPoints(nextBalance);
      return true;
    }catch(err){
...
function Study(){
 ...
 const grade=async w=>{
   ...
   let earned=false;
   const uid=auth.currentUser.uid, subRef=doc(db,'worksheetSubmissions',`${w.id}_${uid}`), rewardRef=doc(db,'worksheetRewards',`${w.id}_${uid}`), userRef=doc(db,'users',uid), txRef=doc(collection(db,'pointTransactions'));
   try{
     await runTransaction(db,async tx=>{
       // 보상 여부는 worksheetRewards 문서만 기준으로 판단한다.
       // 예전 버전에서 submission.rewarded5P만 true로 남은 경우에도 실제 보상 문서가 없으면 정상 지급된다.
       const rewardSnap=await tx.get(rewardRef);
       const shouldReward=correctCount>=threshold&&!rewardSnap.exists();
       const userSnap=shouldReward?await tx.get(userRef):null;
       const submission={worksheetId:w.id,worksheetTitle:w.title,uid,studentName:auth.currentUser.displayName||'학생',answers:mine,results:detail,correctCount,total:key.length,rewardThreshold:threshold,rewarded5P:rewardSnap.exists()||shouldReward,updatedAt:serverTimestamp()};
       ...
       if(shouldReward){
         ...
         tx.set(txRef,{uid,amount:5,reason:`학습방 1회 보상 · ${w.title}`,balanceAfter:next,createdAt:serverTimestamp()});
         earned=true;
       }
     });
   }catch(err){
     console.error('학습방 포인트 적립 오류',err);
     alert(`채점 결과는 확인했지만 포인트 적립 중 오류가 났어요.\n${err?.message||'잠시 후 다시 시도해주세요.'}`);
     return;
   }
...
function Quiz(){
  ...
  useEffect(()=>{
    ...
    getDoc(doc(db,'quizDailyRewards',`${uid}_${todayKey()}`))
  ...
  const finish=async()=>{
    ...
    if(score!==5){alert(`${score}/5 정답이에요.`);return;}
    const uid=auth.currentUser.uid;
    const date=todayKey();
    ...
    try{
      await runTransaction(db,async tx=>{
        ...
        tx.set(txRef,{uid,amount:3,reason:`퀴즈방 오늘의 보상 · ${date}`,balanceAfter:next,createdAt:serverTimestamp()});
        earned=true;
      });
    }catch(err){
      console.error('퀴즈 포인트 적립 오류',err);
      alert(`채점은 완료했지만 포인트 적립 중 오류가 났어요.\n${err?.message||'잠시 후 다시 시도해주세요.'}`);
      return;
    }
    if(earned){
      setRewardedToday(true);
      alert('5문제 모두 정답! 오늘의 퀴즈 +3P 🎉');
    }else{
      setRewardedToday(true);
      alert('5문제 모두 정답! 🎉\n오늘은 이미 퀴즈 포인트를 받았어요. 내일 다시 +3P를 받을 수 있어요!');
    }
  };
  return <><PageHead title="퀴즈방"/><div className="quiz-reward-banner">🏆 <b>5문제를 모두 맞히면 하루에 한 번 +3P!</b><span>{checking?'오늘의 보상 확인 중...':rewardedToday?'오늘의 +3P는 이미 받았어요 ✓':'오늘 아직 +3P를 받을 수 있어요 ✨'}</span></div><div className="warning-box">⚠️ <b>친구에게 정답을 알려주지 않습니다.</b></div><div className="quiz-meta"><strong>오늘의 교과 퀴즈 (5문제)</strong><span>{idx+1} / 5</span></div><div className="quiz-card"><span className="subject-pill">{q.subject}</span><h2>{q.q}</h2>{q.options.map(o=><label className={`quiz-option ${selected[idx]===o?'selected':''}`} key={o}><input type="radio" checked={selected[idx]===o} onChange={()=>setSelected({...selected,[idx]:o})}/>{o}</label>)}</div><button className="primary-wide" onClick={()=>idx<4?setIdx(idx+1):finish()}>{idx<4?'다음 문제':'채점하기'}</button></>;
}
...
function TeacherPointManager(){
  ...
  const saveAdminPointLog=async(data)=>{
    try{
      ...
      // 포인트 자체 변경과 내역 기록을 분리합니다.
      // 내역 권한에 문제가 있어도 학생 포인트 변경이 되돌아가지 않게 합니다.
      console.error('포인트 내역 기록 오류',err);
      return false;
    }
  };

  const changePoints=async(u,mode)=>{
    const isDeduct=mode==='deduct';
    const label=isDeduct?'차감':'적립';
    const raw=window.prompt(`${u.name||u.email||'학생'}에게 ${label}할 포인트를 입력해주세요.\n예: 5`,'');
    if(raw===null)return;
    const n=Math.max(0,Math.trunc(Number(raw)||0));
    if(!n)return alert('1 이상의 숫자를 입력해주세요.');
    const reason=window.prompt(`${label} 사유를 입력해주세요.`,isDeduct?'교사 포인트 차감':'교사 포인트 적립');
    if(reason===null||!reason.trim())return;
    if(!window.confirm(`${u.name||u.email||'학생'}의 포인트를 ${n}P ${label}할까요?`))return;

    const userRef=doc(db,'users',u.id);
    let before=0,after=0;
    try{
      // 1) 학생의 실제 users.points부터 독립적으로 변경합니다.
      // pointTransactions 쓰기 권한 때문에 전체 작업이 실패하지 않도록 로그와 분리했습니다.
      await runTransaction(db,async tx=>{
        const snap=await tx.get(userRef);
        if(!snap.exists())throw new Error('학생 정보를 찾을 수 없어요.');
        before=Number(snap.data()?.points)||0;
        if(isDeduct&&before<n)throw new Error(`현재 ${before}P라서 ${n}P를 차감할 수 없어요.`);
        const amount=isDeduct?-n:n;
        after=before+amount;
        tx.update(userRef,{points:after});
      });

      // 2) 변경 성공 후 내역은 별도로 기록합니다.
      const amount=isDeduct?-n:n;
      const logged=await saveAdminPointLog({
        uid:u.id,
        amount,
        reason:reason.trim(),
        balanceBefore:before,
        balanceAfter:after,
        type:isDeduct?'teacherDeduction':'teacherAddition'
      });
      alert(`${u.name||u.email||'학생'} 포인트가 ${before}P → ${after}P로 반영되었습니다.${logged?'':'\n(포인트는 반영됐지만 내역 기록 권한은 확인이 필요합니다.)'}`);
    }catch(err){
      console.error('교사 포인트 조정 오류',err);
      alert(`포인트 ${label}에 실패했습니다.\n${err?.message||'잠시 후 다시 시도해주세요.'}`);
    }
  };

  const reverse=async t=>{
    if(!t.amount)return;
    if(reversedIds.has(t.id))return alert('이미 취소한 내역입니다.');
    if(!window.confirm(`이 내역을 반대로 조정할까요?\n${t.amount>0?'+':''}${t.amount}P → ${-t.amount>0?'+':''}${-t.amount}P`))return;

    const userRef=doc(db,'users',t.uid);
    let before=0,after=0;
    try{
      // 기존 내역을 수정/삭제하지 않고 학생의 실제 포인트만 먼저 보정합니다.
      await runTransaction(db,async tx=>{
        const snap=await tx.get(userRef);
        if(!snap.exists())throw new Error('학생 정보를 찾을 수 없어요.');
        before=Number(snap.data()?.points)||0;
        ...
        after=before+amount;
        if(after<0)throw new Error(`현재 ${before}P라서 이 내역을 취소하면 포인트가 0P 아래로 내려갑니다.`);
        tx.update(userRef,{points:after});
      });
      ...
      const logged=await saveAdminPointLog({
        uid:t.uid,
        amount,
        reason:`교사 내역 취소 · ${t.reason||'포인트 내역'}`,
        balanceBefore:before,
        balanceAfter:after,
        reversesTransactionId:t.id,
        type:'teacherReversal'
      });
      alert(`학생 포인트가 ${before}P → ${after}P로 반영되었습니다.${logged?'':'\n(포인트는 반영됐지만 취소 내역 기록 권한은 확인이 필요합니다.)'}`);
    }catch(err){
      console.error('포인트 내역 취소 오류',err);
      alert(`내역 취소에 실패했습니다.\n${err?.message||'잠시 후 다시 시도해주세요.'}`);
    }
  };

  const filtered=selectedUid?transactions.filter(t=>t.uid===selectedUid):transactions;
  const reversedIds=new Set(transactions.filter(t=>t.reversesTransactionId).map(t=>t.reversesTransactionId));
  return <div className="teacher-classpoint-grid"><div className="teacher-card"><div className="teacher-section-head"><div><h2>💰 학생 개인 포인트 관리</h2><p>학생별 현재 포인트를 확인하고 적립과 차감을 직접 처리합니다. 변경 즉시 학생 화면의 현재 포인트에도 반영됩니다.</p></div><span className="live-badge">● 실시간</span></div><div className="point-student-grid">{users.map(u=><div className="point-student-card" key={u.id}><div><strong>{u.name||u.email||'학생'}</strong><span>{Number(u.points||0)}P</span></div><div className="admin-actions"><button className="small-action" onClick={()=>changePoints(u,'add')}>＋ 적립</button><button className="danger-btn" onClick={()=>changePoints(u,'deduct')}>－ 차감</button></div></div>)}</div></div><div className="teacher-card"><div className="teacher-section-head"><div><h2>🧾 포인트 내역</h2><p>잘못된 내역은 삭제하지 않고 반대 금액의 새 기록을 만들어 학생의 실제 포인트를 즉시 바로잡습니다.</p></div><select value={selectedUid} onChange={e=>setSelectedUid(e.target.value)}><option value="">전체 학생</option>{users.map(u=><option key={u.id} value={u.id}>{u.name||u.email||'학생'}</option>)}</select></div>{filtered.length?<div className="teacher-suggestion-list">{filtered.slice(0,200).map(t=>{const u=users.find(x=>x.id===t.uid),isReversed=reversedIds.has(t.id);return <div className="teacher-suggestion-item" key={t.id}><div className="suggestion-main"><div className="suggestion-meta"><strong>{u?.name||u?.email||t.uid||'학생'}</strong><span>{formatCreatedAt(t.createdAt)}</span></div><p>{t.reason||'포인트 내역'} · 잔액 {Number(t.balanceAfter||0)}P</p></div><div className="point-history-admin"><b className={Number(t.amount)>=0?'plus':'minus'}>{Number(t.amount)>0?'+':''}{Number(t.amount)}P</b>{isReversed?<span className="live-badge">취소 완료</span>:<button className="small-action" onClick={()=>reverse(t)}>이 내역 취소</button>}</div></div>})}</div>:<Empty text="포인트 내역이 없어요."/>}</div></div>;
}
...
function TeacherClassPoints(){
  ...
  const addClassPoints=async()=>{const n=Math.max(0,Math.floor(Number(addAmount)||0));if(!n)return alert('추가할 포인트를 입력해주세요.');if(!addReason.trim())return alert('추가 사유를 입력해주세요.');if(!window.confirm(`학급 전체 포인트에 ${n}P를 추가할까요?`))return;await addDoc(collection(db,'classPointContributions'),{uid:auth.currentUser.uid,studentName:'선생님',amount:n,roundId:classPoint.rewardRoundId||1,reason:addReason.trim(),type:'teacherAddition',createdAt:serverTimestamp()});setAddAmount('');setAddReason('');alert(`${n}P 추가했습니다.`)};
