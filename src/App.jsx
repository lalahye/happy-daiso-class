...
function Study(){
 ...
 const grade=async w=>{
   ...
   await runTransaction(db,async tx=>{
     const [subSnap,rewardSnap]=await Promise.all([tx.get(subRef),tx.get(rewardRef)]);
     const oldRewarded=rewardSnap.exists()||!!subSnap.data()?.rewarded5P;
...
function TeacherPointManager(){
  ...
  const adjust=async u=>{const raw=window.prompt(`${u.name||u.email||'학생'}의 포인트를 조정합니다.\n추가: 5   차감: -5`,'');if(raw===null)return;const amount=Math.trunc(Number(raw));if(!amount)return alert('0이 아닌 숫자를 입력해주세요.');const reason=window.prompt('조정 사유를 입력해주세요.','교사 포인트 조정');if(reason===null||!reason.trim())return;const userRef=doc(db,'users',u.id),txRef=doc(collection(db,'pointTransactions'));await runTransaction(db,async tx=>{const snap=await tx.get(userRef);const current=Number(snap.data()?.points)||0,next=current+amount;tx.update(userRef,{points:next});tx.set(txRef,{uid:u.id,amount,reason:reason.trim(),balanceAfter:next,createdAt:serverTimestamp(),adjustedBy:auth.currentUser.uid})});alert(`${amount>0?'+':''}${amount}P 조정했습니다.`)};
  const reverse=async t=>{if(!t.amount)return;if(!window.confirm(`이 내역을 반대로 조정할까요?\n${t.amount>0?'+':''}${t.amount}P → ${-t.amount>0?'+':''}${-t.amount}P`))return;const userRef=doc(db,'users',t.uid),txRef=doc(collection(db,'pointTransactions'));await runTransaction(db,async tx=>{const snap=await tx.get(userRef);const current=Number(snap.data()?.points)||0,next=current-Number(t.amount);tx.update(userRef,{points:next});tx.set(txRef,{uid:t.uid,amount:-Number(t.amount),reason:`교사 내역 취소 · ${t.reason||'포인트 내역'}`,balanceAfter:next,createdAt:serverTimestamp(),adjustedBy:auth.currentUser.uid,reversesTransactionId:t.id})});alert('반대 조정 내역을 기록했습니다.');};
  const filtered=selectedUid?transactions.filter(t=>t.uid===selectedUid):transactions;
  return <div className="teacher-classpoint-grid"><div className="teacher-card"><div className="teacher-section-head"><div><h2>💰 학생 개인 포인트 관리</h2><p>학생별 현재 포인트를 확인하고 잘못 적립·차감된 포인트를 바로 조정할 수 있습니다.</p></div><span className="live-badge">● 실시간</span></div><div className="point-student-grid">{users.map(u=><div className="point-student-card" key={u.id}><div><strong>{u.name||u.email||'학생'}</strong><span>{Number(u.points||0)}P</span></div><button onClick={()=>adjust(u)}>± 포인트 조정</button></div>)}</div></div><div className="teacher-card"><div className="teacher-section-head"><div><h2>🧾 포인트 내역</h2><p>잘못된 내역은 삭제하지 않고 반대 금액을 기록해 잔액과 기록을 함께 맞춥니다.</p></div><select value={selectedUid} onChange={e=>setSelectedUid(e.target.value)}><option value="">전체 학생</option>{users.map(u=><option key={u.id} value={u.id}>{u.name||u.email||'학생'}</option>)}</select></div>{filtered.length?<div className="teacher-suggestion-list">{filtered.slice(0,200).map(t=>{const u=users.find(x=>x.id===t.uid);return <div className="teacher-suggestion-item" key={t.id}><div className="suggestion-main"><div className="suggestion-meta"><strong>{u?.name||u?.email||t.uid||'학생'}</strong><span>{formatCreatedAt(t.createdAt)}</span></div><p>{t.reason||'포인트 내역'} · 잔액 {Number(t.balanceAfter||0)}P</p></div><div className="point-history-admin"><b className={Number(t.amount)>=0?'plus':'minus'}>{Number(t.amount)>0?'+':''}{Number(t.amount)}P</b><button className="small-action" onClick={()=>reverse(t)}>이 내역 취소</button></div></div>})}</div>:<Empty text="포인트 내역이 없어요."/>}</div></div>;
}
