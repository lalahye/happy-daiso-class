...
function Study(){
 ...
 const grade=async w=>{
   ...
   await runTransaction(db,async tx=>{
     ...
     if(shouldReward){
       const userSnap=await tx.get(userRef);const current=Number(userSnap.data()?.points)||0;const next=current+5;
...
function TeacherStudy(){
 ...
 const ocrCanvas=async(canvas,lang='eng',base=0,weight=1)=>{const r=await Tesseract.recognize(canvas,lang,{logger:m=>{if(m.status==='recognizing text')setOcrProgress(Math.min(99,Math.round((base+(m.progress||0)*weight)*100)));}}, {tessedit_pageseg_mode:'6',preserve_interword_spaces:'1'});return r.data.text||''};
 const analyzeImageSheet=async file=>{const canvas=await toCanvas(file);let text=await ocrCanvas(canvas,'eng',0,.55);let found=parseAnswerText(text);const expected=count||Math.max(0,...Object.keys(found).map(Number));if(Object.keys(found).length<Math.max(3,Math.floor((expected||10)*.5))){text+='\n'+await ocrCanvas(canvas,'kor+eng',.55,.45);found={...parseAnswerText(text),...found};}return found};
