...
function Study(){
 ...
 const grade=async w=>{
   ...
   let earned=false;
   const uid=auth.currentUser.uid, subRef=doc(db,'worksheetSubmissions',`${w.id}_${uid}`), userRef=doc(db,'users',uid), txRef=doc(collection(db,'pointTransactions'));
   await runTransaction(db,async tx=>{
     const subSnap=await tx.get(subRef), oldRewarded=!!subSnap.data()?.rewarded5P;
     const shouldReward=correctCount>=threshold&&!oldRewarded;
     const userSnap=shouldReward?await tx.get(userRef):null;
     const submission={worksheetId:w.id,worksheetTitle:w.title,uid,studentName:auth.currentUser.displayName||'학생',answers:mine,results:detail,correctCount,total:key.length,rewardThreshold:threshold,rewarded5P:oldRewarded||shouldReward,updatedAt:serverTimestamp()};
     tx.set(subRef,submission,{merge:true});
     if(shouldReward){
       const current=Number(userSnap?.data()?.points)||0;const next=current+5;
       tx.update(userRef,{points:next});
       tx.set(txRef,{uid,amount:5,reason:`학습방 목표 달성 · ${w.title}`,balanceAfter:next,createdAt:serverTimestamp()});
       earned=true;
     }
   });
...
function TeacherStudy(){
 ...
 const normalizeOcrText=text=>(text||'').replace(/\r/g,'\n').replace(/[①❶]/g,'1').replace(/[②❷]/g,'2').replace(/[③❸]/g,'3').replace(/[④❹]/g,'4').replace(/[⑤❺]/g,'5').replace(/[—–−]/g,'-').replace(/[|]/g,'/');
 const parseAnswerText=text=>{
   const clean=normalizeOcrText(text),found={};
   const lines=clean.split(/\n+/).map(x=>x.trim()).filter(Boolean);
   for(const line of lines){
     const m=line.match(/^\s*0?(\d{1,3})\s*(?:번|[.)、:\-])?\s*[:.)-]?\s*(.+?)\s*$/);
     if(m){const no=Number(m[1]);let ans=m[2].trim().replace(/\s*([/=])\s*/g,'$1');if(no>0&&no<=100&&ans&&!/^\d{1,3}$/.test(ans))found[no]=ans;}
   }
   const joined=clean.replace(/\n/g,'  ');
   const re=/(?:^|\s)0?(\d{1,3})\s*(?:번|[.)、:\-])\s*([^\n]+?)(?=(?:\s+0?\d{1,3}\s*(?:번|[.)、:\-]))|$)/g;
   let m;while((m=re.exec(joined))){const no=Number(m[1]);const ans=m[2].trim().replace(/\s*([/=])\s*/g,'$1');if(no>0&&no<=100&&ans&&!found[no])found[no]=ans;}
   return found;
 };
 const toCanvas=async file=>{const url=URL.createObjectURL(file);try{const img=new Image();await new Promise((res,rej)=>{img.onload=res;img.onerror=rej;img.src=url});const scale=Math.max(2,1800/img.width);const c=document.createElement('canvas');c.width=Math.round(img.width*scale);c.height=Math.round(img.height*scale);const ctx=c.getContext('2d');ctx.drawImage(img,0,0,c.width,c.height);const im=ctx.getImageData(0,0,c.width,c.height);for(let i=0;i<im.data.length;i+=4){const g=.299*im.data[i]+.587*im.data[i+1]+.114*im.data[i+2];const v=g>190?255:g<110?0:g;im.data[i]=im.data[i+1]=im.data[i+2]=v;}ctx.putImageData(im,0,0);return c;}finally{URL.revokeObjectURL(url)}};
 const ocrCanvas=async(canvas,lang='eng',base=0,weight=1,extra={})=>{const r=await Tesseract.recognize(canvas,lang,{logger:m=>{if(m.status==='recognizing text')setOcrProgress(Math.min(99,Math.round((base+(m.progress||0)*weight)*100)));},tessedit_pageseg_mode:'6',preserve_interword_spaces:'1',...extra});return r.data.text||''};
 const cropCanvas=(src,x,y,w,h)=>{const c=document.createElement('canvas');c.width=Math.max(1,Math.round(w));c.height=Math.max(1,Math.round(h));const ctx=c.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(src,Math.max(0,x),Math.max(0,y),Math.min(w,src.width-Math.max(0,x)),Math.min(h,src.height-Math.max(0,y)),0,0,c.width,c.height);return c};
 const ocrDigits=async(c,base=0,weight=.02)=>{const text=await ocrCanvas(c,'eng',base,weight,{tessedit_pageseg_mode:'7',tessedit_char_whitelist:'0123456789'});const m=(text||'').match(/\d{1,3}/g);return m?m.join('').slice(0,3):''};
 const detectFractionBars=canvas=>{
   const ctx=canvas.getContext('2d'),im=ctx.getImageData(0,0,canvas.width,canvas.height).data,w=canvas.width,h=canvas.height;
   const dark=(x,y)=>{if(x<0||x>=w||y<0||y>=h)return false;return im[(y*w+x)*4]<90};
   const raw=[];const minW=Math.max(10,Math.round(w*.006)),maxW=Math.max(40,Math.round(w*.12));
   for(let y=1;y<h-1;y+=2){let x=0;while(x<w){while(x<w&&!dark(x,y))x++;const start=x;while(x<w&&dark(x,y))x++;const len=x-start;if(len>=minW&&len<=maxW)raw.push({x:start,y,w:len});}}
   const merged=[];
   for(const r of raw){let m=merged.find(b=>Math.abs(b.y-r.y)<=5&&Math.abs(b.x-r.x)<=Math.max(8,b.w*.35)&&Math.abs(b.w-r.w)<=Math.max(12,b.w*.55));if(m){m.x=Math.min(m.x,r.x);m.w=Math.max(m.x+m.w,r.x+r.w)-m.x;m.y=(m.y+r.y)/2;m.h=(m.h||2)+2}else merged.push({...r,h:2});}
   const ink=(x1,y1,x2,y2)=>{let n=0,total=0;const sx=Math.max(0,Math.floor(x1)),ex=Math.min(w,Math.ceil(x2)),sy=Math.max(0,Math.floor(y1)),ey=Math.min(h,Math.ceil(y2));for(let yy=sy;yy<ey;yy+=3)for(let xx=sx;xx<ex;xx+=3){total++;if(dark(xx,yy))n++;}return total?n/total:0};
   return merged.filter(b=>{const bh=Math.max(24,b.w*1.3),pad=b.w*.25;const above=ink(b.x-pad,b.y-bh,b.x+b.w+pad,b.y-3),below=ink(b.x-pad,b.y+3,b.x+b.w+pad,b.y+bh);return above>.008&&below>.008;});
 };
 const recognizeVerticalFractions=async canvas=>{
   const bars=detectFractionBars(canvas);if(!bars.length)return {};
   const yTol=Math.max(28,canvas.height*.018),nearX=Math.max(120,canvas.width*.09);const groups=[];
   for(const b of bars.sort((a,b)=>a.y-b.y||a.x-b.x)){
     let g=groups.find(g=>Math.abs(g.y-b.y)<=yTol&&b.x-(g.maxX||g.x)<=nearX&&b.x>=g.x-nearX*.25);
     if(!g){g={x:b.x,y:b.y,maxX:b.x+b.w,bars:[]};groups.push(g)}g.bars.push(b);g.x=Math.min(g.x,b.x);g.maxX=Math.max(g.maxX,b.x+b.w);g.y=g.bars.reduce((a,z)=>a+z.y,0)/g.bars.length;
   }
   const useful=groups.filter(g=>g.bars.length<=4);
   const cols=[];for(const g of useful.sort((a,b)=>a.x-b.x)){let c=cols.find(c=>Math.abs(c.x-g.x)<=canvas.width*.11);if(!c){c={x:g.x,groups:[]};cols.push(c)}c.groups.push(g);c.x=c.groups.reduce((a,z)=>a+z.x,0)/c.groups.length;}
   const ordered=cols.sort((a,b)=>a.x-b.x).flatMap(c=>c.groups.sort((a,b)=>a.y-b.y));
   const found={};let done=0,total=Math.max(1,ordered.reduce((n,g)=>n+g.bars.length,0));
   for(let qi=0;qi<ordered.length;qi++){
     const vals=[];
     for(const b of ordered[qi].bars.sort((a,b)=>a.x-b.x)){
       const hh=Math.max(32,b.w*1.45),pad=Math.max(5,b.w*.18),x=b.x-pad,ww=b.w+pad*2;
       const top=cropCanvas(canvas,x,b.y-hh,ww,hh-4),bottom=cropCanvas(canvas,x,b.y+4,ww,hh-4);
       const num=await ocrDigits(top,.05+done/total*.65,.65/total);done++;const den=await ocrDigits(bottom,.05+done/total*.65,.65/total);
       if(num&&den)vals.push(`${num}/${den}`);
     }
     const unique=[...new Set(vals)];if(unique.length)found[qi+1]=unique.join(' | ');
   }
   return found;
 };
 const analyzeImageSheet=async file=>{const canvas=await toCanvas(file);const fractionFound=await recognizeVerticalFractions(canvas);let text=await ocrCanvas(canvas,'eng',.72,.16);let found=parseAnswerText(text);const expected=count||Math.max(Object.keys(fractionFound).length,Math.max(0,...Object.keys(found).map(Number)));if(Object.keys(found).length<Math.max(3,Math.floor((expected||10)*.35))){text+='\n'+await ocrCanvas(canvas,'kor+eng',.88,.11);found={...parseAnswerText(text),...found};}return {...found,...fractionFound}};
 const analyzeAnswerSheet=async file=>{
   ...
   try{
     ...
     if(file.type==='application/pdf'){
       ...
       let allText='';
       for(let i=1;i<=pdf.numPages;i++){const page=await pdf.getPage(i),c=await page.getTextContent();allText+='\n'+c.items.map(x=>x.str).join(' ');setOcrProgress(Math.round(i/pdf.numPages*45));}
       found=parseAnswerText(allText);
       if(Object.keys(found).length<3){
         let imageText='';for(let i=1;i<=pdf.numPages;i++){const page=await pdf.getPage(i),viewport=page.getViewport({scale:3});const canvas=document.createElement('canvas');canvas.width=viewport.width;canvas.height=viewport.height;await page.render({canvasContext:canvas.getContext('2d'),viewport}).promise;imageText+='\n'+await ocrCanvas(canvas,'eng',.45+(i-1)/pdf.numPages*.5,.5/pdf.numPages);}found={...parseAnswerText(imageText),...found};
       }
     }else found=await analyzeImageSheet(file);
     const nums=Object.keys(found).map(Number).sort((a,b)=>a-b);
     if(!nums.length)return alert('답안 번호를 찾지 못했어요. 번호와 답이 또렷하게 보이는 이미지로 다시 시도해주세요.');
     const n=Math.max(count||0,...nums);setCount(n);setRows(Array.from({length:n},(_,i)=>({no:i+1,text:found[i+1]||''})));setRewardThreshold(v=>v||String(Math.ceil(n*.8)));setOcrProgress(100);
     alert(`${nums.length}개 정답을 자동으로 찾았어요.\n분수형 답안지는 OCR 특성상 일부가 틀릴 수 있으니 빈칸·분수만 한 번 확인해주세요.`);
   }catch(e){console.error(e);alert('답안지 자동 인식에 실패했어요. 이미지가 선명한지 확인해주세요.');}finally{setOcrBusy(false)}
 };
 const clearForm=()=>{setTitle('');setPdfUrl('');setCount(0);setRows([]);setRewardThreshold('');setEditingId('')};
 const submit=async e=>{e.preventDefault();const answers=rows.map(r=>({no:r.no,accepted:r.text.split('|').map(x=>x.trim()).filter(Boolean)}));if(answers.some(a=>!a.accepted.length))return alert('빈 정답이 있어요. OCR 결과에서 비어 있거나 잘못 인식된 부분만 수정해주세요.');const threshold=Math.max(1,Math.min(answers.length,Number(rewardThreshold)||answers.length));const data={title,pdfUrl,questionCount:answers.length,answers,rewardThreshold:threshold,rewardPoints:5,published:true,updatedAt:serverTimestamp()};if(editingId){await updateDoc(doc(db,'worksheets',editingId),data);alert('문제지를 수정했습니다.')}else{await addDoc(collection(db,'worksheets'),{...data,createdAt:serverTimestamp()});alert('등록 완료! 학생 학습방에 바로 나타납니다.')}clearForm()};
 const editWorksheet=w=>{setEditingId(w.id);setTitle(w.title||'');setPdfUrl(w.pdfUrl||'');const a=(w.answers||[]).map(x=>({no:x.no,text:(x.accepted||[]).join(' | ')}));setCount(w.questionCount||a.length||1);setRows(a.length?a:Array.from({length:w.questionCount||1},(_,i)=>({no:i+1,text:''})));setRewardThreshold(String(w.rewardThreshold||w.questionCount||a.length||1));window.scrollTo({top:0,behavior:'smooth'})};
