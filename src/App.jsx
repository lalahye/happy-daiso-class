...
function TeacherPointManager(){
  ...
  const changePoints=async(u,mode)=>{
    ...
    const userRef=doc(db,'users',u.id),txRef=doc(collection(db,'pointTransactions'));
    ...
    try{
      await runTransaction(db,async tx=>{
        ...
        tx.set(txRef,{uid:u.id,amount,reason:reason.trim(),balanceBefore:before,balanceAfter:after,createdAt:serverTimestamp(),adjustedBy:auth.currentUser.uid,type:isDeduct?'teacherDeduction':'teacherAddition'});
      });
      alert(`${u.name||u.email||'학생'} 포인트가 ${before}P → ${after}P로 바로 반영되었습니다.`);
    }catch(err){
  ...
  const reverse=async t=>{
    ...
    const userRef=doc(db,'users',t.uid),txRef=doc(collection(db,'pointTransactions'));
    let before=0,after=0;
    try{
      await runTransaction(db,async tx=>{
        const snap=await tx.get(userRef);
        ...
        tx.update(userRef,{points:after});
        tx.set(txRef,{uid:t.uid,amount,reason:`교사 내역 취소 · ${t.reason||'포인트 내역'}`,balanceBefore:before,balanceAfter:after,createdAt:serverTimestamp(),adjustedBy:auth.currentUser.uid,reversesTransactionId:t.id,type:'teacherReversal'});
      });
      alert(`학생 포인트가 ${before}P → ${after}P로 바로 반영되었습니다.`);
    }catch(err){
